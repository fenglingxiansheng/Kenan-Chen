...
def compute_interface_loss(u_net1,u_net2, X, phi, Phi, flag):
    ...
    if flag:
        ...
        r = torch.sqrt(torch.pow(X[:, 0:1], 2) + torch.pow(X[:, 1:2], 2))
...
def compute_loss(u1_net, u2_net, X_inside, X_d, X_gamma, phi, Phi , area):
    ...
    if area:
        pde_loss = compute_pde_loss(u1_net, X_inside, 1)
        # 边界损失
        ...
        pde_loss = compute_pde_loss(u2_net, X_inside, 0)
...
for k in range(max_outer_iter):
    ...
    with torch.no_grad():  # 全局禁用梯度
        ...
        r = torch.sqrt(torch.pow(X_gamma[:, 0:1], 2) + torch.pow(X_gamma[:, 1:2], 2))
        dr_dtheta = r1 * OMEGA * torch.cos(OMEGA * theta)
        # 计算切向量分量 (dx/dθ, dy/dθ)
        dx_dtheta = dr_dtheta * torch.cos(theta) - r * torch.sin(theta)
        dy_dtheta = dr_dtheta * torch.sin(theta) + r * torch.cos(theta)
        ...
        # Phi = pred_data(u1_net, X_gamma) - pred_data(u2_net, X_gamma)
...
def plot_results(u1_net, u2_net):
    ...
    plt.tight_layout()
    plt.savefig('test_domain_PINN - 3.png')
